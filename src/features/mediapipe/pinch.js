...
export default {
  ...
  onFrame(hands) {
    ...
    const height = hands.image.height;
    ...
    return this.getPinchStates(hands, leftVisible, rightVisible);
  },
    ...
    for (let hand = 0; hand < 2; hand++) {
      for (let finger = 0; finger < 4; finger++) {
        ...
        if (
          ...
          hands.pinchState[hand][finger] = POINTER_STATE.START;
        } else if (
          this.pinchDowned[hand][finger] > this.config.maxMouseDownedFrames
        ) {
          hands.pinchState[hand][finger] = POINTER_STATE.HELD;
        } else if (this.pinchUp[hand][finger]) {
          hands.pinchState[hand][finger] = POINTER_STATE.RELEASE;
        } else {
          hands.pinchState[hand][finger] = POINTER_STATE.OPEN;
        }
      }
    }
